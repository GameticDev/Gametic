...
const UserList = () => {
  ...
  const { users, totalUsers } = useAppSelector((state) => state.adminUsers);
  useEffect(() => {
    dispatch(fetchAllUser({ page: currentPage, limit: 5 }));
  }, [dispatch, currentPage]);

  const blockUserById = async (id: string): Promise<User | undefined> => {
    try {
      const response = await dispatch(blockUser({ id: id })).unwrap();
      dispatch(fetchAllUser({ page: currentPage, limit: 5 }));
      alert("ok");
    } catch (error: unknown) {
      ...
      <table className="w-full">
        ...
        <tbody>
          {users.map((item, index) => {
            return (
              <tr className=" hover:bg-[#f4f5f6] " key={index}>
                <td className="text-black p-4 flex items-center">
                  <span className="mr-2">
