...
//   const [filteredTurfs, setFilteredTurfs] = useState<string[]>([]);
...
//         .filter(turf => typeof turf.name === "string" && turf.name.toLowerCase().includes(query.toLowerCase()))
//         .map(turf => turf.name);
...
//           <h1 className="text-xl font-semibold text-[#00423D]">Owner Portal</h1>
//         </div>
...
//               className="border border-gray-300 rounded-lg px-3 py-1 text-sm focus:outline-none focus:ring-1 focus:ring-[#00423D]"
//             />
//             <Search className="absolute right-2 top-1/2 transform -translate-y-1/2 text-[#667085] w-4 h-4" />

//             {/* Dropdown Results */}
//             {showDropdown && filteredTurfs.length > 0 && (
//               <ul className="absolute z-50 bg-white border border-gray-200 w-full mt-1 rounded-md shadow-md max-h-48 overflow-auto">
//                 {filteredTurfs.map((name, index) => (
//                   <li
//                     key={index}
