...
// //     if (user?._id) {
// //       await dispatch(fetchTurfs(user._id));
// //     }
// //   };
...
const [existingImages, setExistingImages] = useState<(string | File)[]>([]);
  ...
  setPreviewImages(turfToEdit.images);
        ...
        // else if (typeof turfToEdit.images === 'string') {
        //   const imagesArray = [turfToEdit.images];
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setPreviewImages((prev) => [...prev, ...newPreviewUrls]);
    setFiles((prev) => [...prev, ...fileArray]);
    setValue('images', [...files, ...fileArray]);
  };
  ...
  const validateCurrentStep = async () => {
    ...
    switch (step) {
      case 1:
        fieldsToValidate = ['name', 'city', 'area'];
        break;
      case 2:
